...
namespace ConDep.Console
{
  public class CmdDeployValidator : CmdBaseValidator<ConDepOptions>
  {
    ...
    {
      if (string.IsNullOrWhiteSpace(options.CryptoKey)) return;
    ...
    private static void ValidateEnvironment(ConDepOptions options)
    {
    ...
    private static void ValidateAssemblyName(ConDepOptions options)
    {
    ...
    private static void ValidateRunbook(ConDepOptions options)
    {
      if (options.DeployAllApps) return;
      if (!string.IsNullOrWhiteSpace(options.Runbook)) return;

      throw new ConDepCmdParseException(
        "No Runbook provided. If you want to execute all Runbooks, use the --deployAllApps switch.");
    }
  }
}