...
namespace ConDep.Execution
{
  public class ConDepConfigurationExecutor : MarshalByRefObject, ITokenSource, IDisposable
  {
    ...
    public ConDepExecutionResult Execute(Guid executionId, string assemblyPath, ConDepOptions options,
      ITokenSource token)
    {
    ...
    public static ConDepExecutionResult ExecuteFromAssembly(ConDepSettings conDepSettings, CancellationToken token)
    {
      try
      {
        ...
        var lbLookup = new LoadBalancerLookup(conDepSettings.Config.LoadBalancer);
        var runbookConfigurationHandler = new RunbookConfigurationHandler(new RunbookHandler(),
          new RunbookDependencyHandler(), lbLookup.GetLoadBalancer());
        ...
        return new ConDepConfigurationExecutor().Execute(conDepSettings, clientValidator, serverValidator,
          runbookConfigurationHandler.GetRunbooksToExecute(conDepSettings), new ServerHandler(), token);
      }
      catch (Exception ex)
      {
        Logger.Error("An error sneaked by.", ex);
    ...
    public static Task<ConDepExecutionResult> ExecuteFromAssemblyAsync(ConDepSettings conDepSettings,
