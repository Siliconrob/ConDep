...
namespace ConDep.Execution.Logging
{
  public class TeamCityLogger : Log4NetLoggerBase
  {
    ...
    private void TeamCityMessage(string message, Exception ex, TeamCityMessageStatus status, params object[] formatArgs)
    {
      ...
      var tcMessage = string.Format("##teamcity[message text='{0}' errorDetails='{1}' status='{2}']", formattedMessage,
        errorDetails, status);
    ...
    {
      _serviceMessageLogger.Logger.Log(typeof(Logger), Level.All,
        string.Format("##teamcity[blockOpened name='{0}']", EscapeCharsForTeamCity(name)), null);
    }

    private void TeamCityBlockEnd(string name)
    {
      _serviceMessageLogger.Logger.Log(typeof(Logger), Level.All,
        string.Format("##teamcity[blockClosed name='{0}']", EscapeCharsForTeamCity(name)), null);
    }

    private void TeamCityProgressMessage(string message)
    {
      _serviceMessageLogger.Logger.Log(typeof(Logger), Level.All,
        string.Format("##teamcity[progressMessage '{0}']", EscapeCharsForTeamCity(message)), null);
    }
  }
}