...
namespace ConDep.Execution.Relay
{
  public class RelayHandler
  {
    ...
    public ConDepExecutionResult Relay(ArtifactManifest artifactManifest, RelayConfig relayConfig,
      DeployOptions deployOptions)
    {
      ...
      try
      {
        ...
        if (!result.IsSuccessStatusCode)
        {
          Logger.Error("Failed to retreive execution log! Status code " + result.StatusCode + ". Reason : " +
                       result.ReasonPhrase);
          return new ConDepExecutionResult(false);
        }
        ...
        var statusResult = _client.GetAsync(string.Format("RelayService/Runbook/{0}", startedStatus.ExecutionId))
          .Result;
        if (!statusResult.IsSuccessStatusCode)
        {
          Logger.Error("Failed to get latest status! Status code " + result.StatusCode + ". Reason : " +
                       result.ReasonPhrase);
          return new ConDepExecutionResult(false);
        }
