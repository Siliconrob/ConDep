...
namespace ConDep.Dsl.Remote.Node.Model
{
  public class SyncDirDirectory
  {
    ...
    public IEnumerable<SyncDirFile> Files => _files;
    public IEnumerable<SyncDirDirectory> Directories => _dirs;
    public List<Link> Links { get; } = new List<Link>();

    public bool EqualTo(DirectoryInfo dirInfo, string dirInfoRootPath)
    {
      if (dirInfo == null) return false;
      if (RelativePath.Equals(dirInfo.TrimPath(dirInfoRootPath),
    ...
    private SyncDirFile GetFileByRelativePath(string relativePath)
    {
      foreach (var file in Files.Where(
        file => file.RelativePath.Equals(relativePath, StringComparison.InvariantCultureIgnoreCase)))
        return file;
    ...
    {
      foreach (var dir in Directories.Where(
        dir => dir.RelativePath.Equals(relativePath, StringComparison.InvariantCultureIgnoreCase)))
        return dir;
    ...
    private IEnumerable<SyncPath> GetChangedPaths(IEnumerable<SyncPath> srcPaths, IEnumerable<SyncPath> missingDirPaths,
      string rootPath)
    {
