...
namespace ConDep.Dsl.Remote
{
  ...
  {
    private const string REG_KEY_ALLOW_FRESH_CREDENTIALS =
      @"SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentials";

    private const string REG_KEY_ALLOW_FRESH_CREDENTIALS_WHEN_NTLM_ONLY =
      @"SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly";
    ...
    private void EnableFreshCredentials(string key)
    {
      ...
      var regKey = Registry.LocalMachine.OpenSubKey(key, true);
      if (regKey == null)
      {
        ...
        _cleanupFunctions.Add(() => Registry.LocalMachine.DeleteSubKey(key));
      }
