...
namespace ConDep.Dsl.Remote
{
  public class PowerShellExecutor
  {
    ...
    public IEnumerable<dynamic> ExecuteLocal(ServerConfig localServer, string commandOrScript,
    ...
    private static void ConfigureCommand(string commandOrScript, IEnumerable<CommandParameter> parameters,
      Pipeline pipeline)
    {
      if (parameters != null)
      {
      ...
      PowerShellModulesToLoad modules)
    {
      ...
      {
        var netLibraryCmd =
          $@"Add-Type -Path ""{Path.Combine(folders.PSTempFolder, "ConDep.Dsl.Remote.Helpers.dll")}""";
  ...
  internal class RemoteScriptFolders
  {
    public RemoteScriptFolders(ServerConfig server)
    {
      ...
      PSTempFolder = string.IsNullOrWhiteSpace(serverInfo.TempFolderPowerShell)
        ? string.Empty
        : serverInfo.TempFolderPowerShell;
